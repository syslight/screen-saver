...
智能屏部署不下载 KWS/ASR/TTS 模型，也不保存模型 Key；它只连接 `home_agent`。AILABS
设备由 App 绑定固件原生 AliTVASR 服务；厂商登录不可用时，已 root 的 AILABS_S1L 通过
Magisk 只读订阅 `WakeupManager` 标签接收“天猫精灵”KWS 事件。其他设备在没有原生适配器时
保留空格键、触屏和控制端手动入口。ASR、自动断句、Agent 与 TTS 均在 Home Agent 配置。
...
三平台共同注意：自启场景下确保网络先于应用就绪。天气与 Home Agent 连接需要网络；展示端
首启不下载 KWS、ASR 或 TTS 模型。
