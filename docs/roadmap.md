...
- **依据**：仓库仅有 `linux/`、`macos/`、`windows/` 三个桌面平台目录，无 `android/`、`ios/` 目录；手机端为浏览器访问的单页 `web_console/index.html`（打包进 assets，见 `pubspec.yaml`）。
- **影响**：手机端体验受浏览器限制——不能常驻后台、无系统级分享入口、需手动输入地址或扫码进入。
- **定位**：目标平台为桌面三平台（呼应 NFR-1），手机端以零安装的浏览器控制台为有意取舍。
...
| Android / iOS 原生控制台 | L-5 | 复用现有 HTTP/WebSocket 协议（见 `docs/protocol.md`）开发原生 App，浏览器控制台继续保留 |
