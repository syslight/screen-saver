...
- 家长 Web 首期由 Home Agent 的 `/parent/` 提供，现有智能屏控制台只增加入口；原生家长 App 和微信只预留通知/客户端边界。
- Agent 运行在家庭服务器的独立 Python 服务中；Flutter App 不内置 Agent 推理。
- 模型通过可替换的 provider 适配器调用。原型可使用 OpenAI 兼容视觉 API，后续可切换本地模型或混合模式。

### 2.1 硬件等待期间的阶段 A（2026-07-24 已批准）

- 家长 Web 改由 Home Agent Server 自带的 `/parent/` 提供；现有智能屏控制台仅增加入口。
- 家庭成员在初始化页面录入，不把姓名写死在代码中。
- 图片限制为单张 12 MiB、每次最多 6 张、家庭作业图片总配额 5 GiB。
- 阶段 A 不调用模型；提交明确进入人工审核，不生成模拟 AI 结论。
- 学生 Android App和模型检查在阶段 A 的服务端状态机/API 稳定后继续实现。
...
## 17. 阶段 B 已确认增补（2026-07-24）

- 学生端建立为独立 `student_app/`，不与公共智能屏合并。
- 第一台平板绑定 10 岁孩子；设备不能自行切换孩子，换人由家长撤销后重新配对。
- 学生设备使用独立 key 和独立 API，绝不复用家长 bearer；响应排除参考答案与评分标准。
- 第一版只交付查看、开始、拍照、最多 6 页预览/删除、提交和家长审核结果。
- 同一可信家庭 Wi-Fi 内暂用 HTTP 以尽快跑通，不开放公网；HTTPS 仍是远程接入前置项。
- Android 最低 API 23；真机型号、镜头方向和拍照清晰度在安装验收时记录，不反向写死业务层。
- 视觉模型、语音操作、主动提醒、完整答案授权和学习计划继续延期。
