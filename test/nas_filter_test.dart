...
import 'package:smart_frame/services/nas_filter.dart';
...
void main() {
  ...
  group('nasPhotoAllowed', () {
    test('关键词命中路径任意段则排除', () {
      expect(
        nasPhotoAllowed('/photo/收集截图/a.jpg',
            enabled: true, keywords: defaultKeywords),
        isFalse,
      );
      expect(
        nasPhotoAllowed('/photo/screenshots/b.jpg',
            enabled: true, keywords: defaultKeywords),
        isFalse,
      );
      // 大小写不敏感
      expect(
        nasPhotoAllowed('/Photo/SCREENSHOTS/c.jpg',
            enabled: true, keywords: defaultKeywords),
        isFalse,
      );
    });
    ...
    test('常见截图文件名模式命中则排除', () {
      expect(
        nasPhotoAllowed('Screenshot_20240101_123456.png',
            enabled: true, keywords: defaultKeywords),
        isFalse,
      );
      expect(
        nasPhotoAllowed('Screen Shot 2024-01-01 at 12.00.00.png',
            enabled: true, keywords: defaultKeywords),
        isFalse,
      );
      expect(
        nasPhotoAllowed('screencap-123.png',
            enabled: true, keywords: defaultKeywords),
        isFalse,
      );
    });
    ...
    test('普通照片放行', () {
      expect(
        nasPhotoAllowed('/photo/2024春节/IMG_0001.jpg',
            enabled: true, keywords: defaultKeywords),
        isTrue,
      );
      expect(
    ...
    test('enabled 为 false 时全部放行（含截图）', () {
      expect(
        nasPhotoAllowed('/photo/收集截图/a.jpg',
            enabled: false, keywords: defaultKeywords),
        isTrue,
      );
      expect(
        nasPhotoAllowed('Screenshot_20240101_123456.png',
    ...
    test('自定义 keywords 为替换语义，截图正则独立生效', () {
      // 自定义关键词命中 → 排除
      expect(
        nasPhotoAllowed('/x/自定义目录/d.jpg',
            enabled: true, keywords: ['自定义']),
        isFalse,
      );
      ...
      // 关键词可加回 → 排除
      expect(
        nasPhotoAllowed('/x/截图/e.jpg',
            enabled: true, keywords: ['自定义', '截图']),
        isFalse,
      );
