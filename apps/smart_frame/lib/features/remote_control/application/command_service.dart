...
import 'package:smart_frame/features/voice/application/intent_parser.dart';
  ...
  Future<String> executeIntent(Intent intent) async {
    ...
    switch (intent.type) {
      ...
      case IntentType.date:
        ...
        reply =
            '今天是${now.month}月${now.day}日，${info.week}。'
            '${info.festivals.isNotEmpty ? '今天是${info.festivals.join('、')}。' : ''}';
      case IntentType.lunar:
        final info = calendarInfoFor(now);
        reply =
            '今天是农历${info.lunarDate}，${info.ganzhiYear}年，生肖属${info.shengxiao}。'
      ...
      case IntentType.filter:
        ...
        if (kw.isEmpty) {
          ...
          reply = ids.isEmpty ? '没找到和$kw相关的照片。' : '好的，放$kw的照片，共${ids.length}张。';
        }
